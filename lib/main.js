        ...
        for (var issue of issues.data.values()) {
            ...
            if (exemptLabel && isLabeled(issue, exemptLabel)) {
                ...
                if (wasLastUpdatedBefore(issue, args.daysBeforeClose)) {
                    var lastCommentFilterResult = yield wasLastUpdatedByGivenUserType(client, issue, args.lastUpdatedUserType);
                ...
                if (lastCommentFilterResult.result) {
                    ...
                    core.debug(`Not match last-commented-user-filter. Skipping mark stale.`);
                }
            }
            if (operationsLeft <= 0) {
...
function wasLastUpdatedByGivenUserType(client, issue, lastUpdatedUserType) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const events = yield client.issues.listEvents({
            owner: github.context.repo.owner,
            repo: github.context.repo.repo,
        ...
        if (events.data.length == 0) {
            ...
            const latestEvent = events.data.reduce((prev, current) => prev.created_at > current.created_at ? prev : current);
            latestEventActor = latestEvent.actor.login;
        }
        try {
            operationNumber++;
            const isCollaborator = yield client.repos.checkCollaborator({
                owner: github.context.repo.owner,
                repo: github.context.repo.repo,
            ...
            if (isCollaborator.status === 204) {
                return {
                    result: lastUpdatedUserType === constants_1.Constants.UserType.Collaborator
                        ? true
                        : false,
                ...
                core.debug('Unexpected status code from check collaborator api. Skip last updated user type filter.');
                return {
                    result: true,
                    operations: operationNumber
                };
